...
export interface InterviewModalOptions {
...
export class InterviewModal extends Modal {
	...
	constructor(
		...
		private readonly options: InterviewModalOptions = {},
	) {
		super(app);
	}
	...
	onOpen(): void {
		...
		this.contentEl.createDiv({
			...
			text: 'Do not enter API keys, passwords, tokens, URLs, hosts, ports, or endpoint values. Configure credentials and provider endpoints only in Settings → Command Center.',
		});
		this.history = this.contentEl.createDiv({
			cls: 'cc-onboarding-history',
		});
		this.composer = this.contentEl.createDiv({
			cls: 'cc-onboarding-composer',
		});
		...
		const actions = this.composer.createDiv({
			cls: 'cc-onboarding-actions',
		});
		const skip = actions.createEl('button', {
			text: 'Skip optional question',
		});
		skip.addEventListener(
			'click',
			() =>
				void this.submit(
					'Skip this optional question; record no invented value and continue with the information already provided.',
				),
		);
		this.send = actions.createEl('button', {
			cls: 'mod-cta',
			text: 'Send',
		});
		...
		void this.engine
			.start()
			.then((message) => this.render('assistant', message))
	...
	private async submit(explicitText?: string): Promise<void> {
		...
		if (containsProtectedSetupInput(text)) {
			...
			this.setStatus(
				'Protected credential or endpoint-like input was blocked locally. Configure it in Settings → Command Center.',
				true,
			);
		...
		await this.render(
			'user',
			explicitText ? '_Optional question skipped._' : text,
		);
		this.setBusy(true);
		...
		try {
			...
			if (reply.synthesis) this.renderApproval(reply.synthesis);
				...
				this.setStatus(
					reply.redacted
						? 'Credential-like content was not sent.'
						: '',
				);
		} catch (error) {
	...
	private renderApproval(synthesis: InterviewSynthesis): void {
		...
		this.approval = this.contentEl.createDiv({
			cls: 'cc-onboarding-approval',
		});
		this.approval.createEl('h3', { text: 'Choose assets to create' });
		this.approval.createEl('p', {
			text: 'Nothing below is created until you confirm. Uncheck anything you do not want.',
		});
		const templateInputs = this.renderChecklist(
			'Templates',
			synthesis.templates,
		);
		const workflowInputs = this.renderChecklist(
			'Workflows',
			synthesis.workflows,
		);
		const actions = this.approval.createDiv({
		...
		back.addEventListener('click', () => {
			...
			this.setStatus(
				'Asset creation cancelled; close or reset the interview to revise answers.',
			);
		});
		const create = actions.createEl('button', {
			text: 'Create selected assets',
			cls: 'mod-cta',
		});
		create.addEventListener('click',  () => { void (async () => {
			...
			back.disabled = true;
			this.setStatus(
				'Creating approved assets and saving configuration…',
			);
			try {
				const result = await this.engine.completeSynthesis(
					selected(templateInputs),
					selected(workflowInputs),
				);
				await this.options.onComplete?.(result.config);
				this.approval?.empty();
				this.approval?.createEl('h3', {
					text: 'Command Center initialized',
				});
				this.approval?.createEl('p', {
					text: `Created ${result.templatePaths.length} template(s) and ${result.workflowPaths.length} workflow(s).`,
				});
				this.setStatus(
					'Configuration and approved assets initialized.',
				);
				new Notice(
					'Command Center configuration and approved assets initialized.',
				);
			} catch (error) {
	...
	>(
		heading: string,
		items: T[],
	): Array<{ id: string; input: HTMLInputElement }> {
		if (!this.approval) return [];
		this.approval.createEl('h4', { text: heading });
		return items.map((item) => {
			const row = this.approval!.createEl('label', {
	...
	private async render(
		...
		const row = this.history.createDiv({
			cls: `cc-onboarding-message cc-onboarding-${role}`,
		});
		await MarkdownRenderer.render(
			this.app,
			content,
			row.createDiv({ cls: 'cc-onboarding-bubble' }),
			'',
			this.renderer,
		);
	...
	private updatePhase(): void {
		...
		const labels: Record<string, string> = {
			topology: 'Vault Topology Discovery',
			'life-map': 'Life Mapping & Active Tracks',
			capacity: 'Metrics, Habits & Dynamic Capacity',
			triage: 'Inbox Triage & Eat the Frog',
			focus: 'Momentum & Focus Rules',
			style: 'Writing Style & Persona Alignment',
			confirmation: 'Review & Confirmation',
			synthesis: 'Blueprint Asset Approval',
		};
		const phase = this.engine.getPhase();
		this.phase.setText(
			phase === 'confirmation' || phase === 'synthesis'
				? (labels[phase] ?? phase)
	...
	private setBusy(value: boolean): void {
		this.busy = value;
		if (this.send) this.send.disabled = value;
...
function selected(
	items: Array<{ id: string; input: HTMLInputElement }>,
): string[] {
